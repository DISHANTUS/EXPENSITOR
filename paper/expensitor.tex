...
asynchronous PostgreSQL store. Financial reasoning (projection, feasibility,
behavioural profiling, savings) is implemented as deterministic Python modules
that compute results from stored facts on read; there is no background scheduler
and no learned model in the numeric path.
...
\begin{table}[t]
\caption{Bank-SMS detection (seed set, 51 messages).}
\label{tab:sms}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
Expensitor (rules) & \textbf{1.000} & 0.871 & \textbf{0.931} & 0.922\\
Baseline (any amount) & 0.660 & 1.000 & 0.795 & 0.686\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\caption{Receipt extraction (seed set, clean text).}
\label{tab:receipt}
\centering
\begin{tabular}{lc}
\toprule
...
\begin{table}[t]
\caption{Reason-suggestion ranking (synthetic held-out probes).}
\label{tab:reasons}
\centering
\begin{tabular}{lccc}
\toprule
...
The amount-aware ranking roughly triples Hit@1 over the baselines, confirming
that the amount gate carries most of the signal. \todo{Replace or supplement with
a real logged-usage evaluation once available.}
...
\todo{repository URL}. All reported numbers are reproducible via a single
command over the checked-in data.
